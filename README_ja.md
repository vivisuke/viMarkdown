...
- **組み込みのDiff（ファイル差分比較）機能**
  別ファイルや編集前後のテキストを2画面で左右比較。ミニマップ表示や差分のワンクリック反映など、文書の推敲や版管理を強力に支援します。
- **カレンダー連携・デイリーノート（日記）機能**
  サイドバーのカレンダーからワンクリックで当日のノートを作成・オープン。テンプレートの自動展開や、ToDoタスクの達成度をカレンダー上で視覚的にトラッキングできます。
- **強力なGrep（一括検索）機能**
...
|0.6|数式表示、マインドマップ？、ページビュー（段組み、脚注）？、プレゼンモード？、矩形選択?、フォルダ表示？、グラフビュー？、遅延ロード、部分変換|2027/01 dev版|
|0.8|マーメイド？, スクリプト言語（JavaScript or Lua）?、コードブロック：各種言語対応|2027/07 dev版？|
|1.0|未定|未定|
...
※ 2026年６月：shigmas 氏がビルドエラーに対処し、Linux環境でもビルド可能にしてくださいました（感謝）。
